...
export const useChampionSpec2 = () => {
    const getSpecById = (id: string) => {
        const result = useQuery(ChampionDetail(id), { fetchPolicy: 'no-cache' })
        if (result.error || result.loading) {
            return null
        }

        const stats = result.data.championDetail.data[id].stats
        return {
            armor: stats.armor || 0,
